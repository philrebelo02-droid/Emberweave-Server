...
function tuneFile(file, opts){
  ...
  for(let i=0;i<enc.length;i++){
    const st=A.applyRewardHero(enc[i]); const s=(opts.nodeOf?opts.nodeOf(st,i):(st.node||i+1));
...
fs.writeFileSync(path.join(os.tmpdir(),'author_rows.json'), JSON.stringify(norm.rows,null,1));
